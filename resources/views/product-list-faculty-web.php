<?php 
    ...
    $facultyId = $_GET['facultybtn'];
    
    $stmt = $mysqli->prepare("SELECT * FROM tbl_products INNER JOIN tbl_faculty ON tbl_products.origin_id = tbl_faculty.faculty_Id WHERE origin_id = '$facultyId'");
    $stmt->execute();

    $arr = $stmt->get_result()->fetch_all(MYSQLI_ASSOC);

    if(!$arr) {
        header('Location: /faculty-list-web');
        exit;
    }
...
<body style="text-align:center;">
    <table class="product-list">
        <tr style="bgcolor: #556B2F;" >
            <h2 style="color: #804444; face: verdana; text-align: center; padding-top: 20px; padding-bottom: 0px"><?php echo $facultySName ?></h2>
        ...
        <tr> 
            <nav style="text-align: center; display: inline-block; background-color:#ffffff;">
                ...
                <form action="/product-list-filter-faculty" method="get" style="border:none; filter:none">
                    ...
                    <button name="prodtype" value="" id="colId" style="cursor:pointer; background-color: transparent; border: none;"><li>Others<span></span><span></span><span></span><span></span></li></button>
            ...
            ?>
                ...
                <form action="/product-details" method="get">
                        ...
                        <div class="card">
                            ...
                            <p style="margin-left: 230px; margin-right: 5px;">
